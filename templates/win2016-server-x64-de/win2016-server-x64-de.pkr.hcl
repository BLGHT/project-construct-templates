...
variable "vm_name" {
  type    = string
  default = "win2016-server-x64-de-template"
}
...
locals {
  template_description = "Windows Server 2016 64-bit (German) template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => localuser:password"
}

source "proxmox-iso" "win2016-server-x64-de" {
...
build {
  sources = ["source.proxmox-iso.win2016-server-x64-de"]
