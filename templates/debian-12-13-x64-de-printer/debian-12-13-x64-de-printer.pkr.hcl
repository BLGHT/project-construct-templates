...
variable "vm_name" {
  type    = string
  default = "debian-12-13-x64-de-printer-template"
}
...
locals {
  template_description = "Debian 12 (German) printer-emulation target built ${legacy_isotime("2006-01-02 03:04:05")} username:password => debian:debian"
}

source "proxmox-iso" "debian12printer" {
  boot_command = [
    "<down><tab>", # non-graphical install
    "<wait>preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian-12-preseed.cfg ",
    "<wait>language=de locale=de_DE.UTF-8 ",
    "<wait>country=DE keymap=de ",
    "<wait>hostname=printer domain=local ",
    "<enter><wait>",
  ]
...
build {
  sources = ["source.proxmox-iso.debian12printer"]

  provisioner "ansible" {
    playbook_file = "ansible/printer-setup.yml"
    use_proxy     = false
