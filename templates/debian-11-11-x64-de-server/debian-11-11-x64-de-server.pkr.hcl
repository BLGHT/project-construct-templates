...
variable "vm_name" {
  type    = string
  default = "debian-11-11-x64-de-server-template"
}
...
locals {
  template_description = "Debian 11 (German) template built ${legacy_isotime("2006-01-02 03:04:05")} username:password => debian:debian"
}

source "proxmox-iso" "debian11" {
  boot_command = [
    "<down><tab>", # non-graphical install
    "<wait>preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/debian-11-preseed.cfg ",
